...
- Should these robustness tests be used on the base analysis or the sensitivity analysis?
- should the robustness analyses be done on the base experiments at all, are they meaningful/will they be written up like that?
- utility of multivariate analysis
...
Undue (influence)[https://rdrr.io/cran/metafor/man/influence.rma.uni.html] of studies on the meta analysed effect size were then assessed.
...
Study 2 was assessed as being an outlier on the basis of undue influence (on all DVs/across all hypotheses, see below) on the predicted average effect size/coefficients (DFFITS, DFBETAS), and the large decrease in heterogeneity on the basis of its removal (tau2.del, QE.del).
...
- Meta analysis: `r meta_results_h1_sensitivity$meta_analysis_results_text`
- Heterogeneity tests: `r meta_results_h1_sensitivity$heterogeneity_test_results_text`
...
- Meta analysis: `r meta_results_h2_sensitivity$meta_analysis_results_text`
- Heterogeneity tests: `r meta_results_h2_sensitivity$heterogeneity_test_results_text`
...
- Meta analysis: `r meta_results_h3_sensitivity$meta_analysis_results_text`
- Heterogeneity tests: `r meta_results_h3_sensitivity$heterogeneity_test_results_text`
...
temp_1 <- results_for_meta_analysis_h1 %>%
  mutate(outcome = "IAT") %>%
  select(Experiment, outcome, yi, sei)

temp_2 <- results_for_meta_analysis_h2 %>%
  mutate(outcome = "self-reported evaluations") %>%
  select(Experiment, outcome, yi, sei)

# convert from log-Odds Ratio to Cohen's d
# https://www.meta-analysis.com/downloads/Meta-analysis%20Converting%20among%20effect%20sizes.pdf
temp_3 <- results_for_meta_analysis_h3 %>%
  mutate(
    # convert log-Odds Ratio to Cohen's d
    # d = logOR * (sqrt(3)/pi)
    yi = yi * (sqrt(3)/pi),
    # convert standard error of log-Odds Ratio to standard error of Cohen's d
    # se_d = se_logOR * (3/pi)
    sei = sei * (sqrt(3)/pi)
  ) %>%
  mutate(outcome = "behavioural intentions") %>%
  select(Experiment, outcome, yi, sei)
...
combined_results <- 
  rbind(temp_1,
        ...
        temp_3) %>%
  mutate(V = sei^2) %>%
  arrange(Experiment, outcome) 

combined_results_trimmed <- combined_results %>%
  filter(Experiment != "Experiment 2")

#write_csv(combined_results_trimmed, "multivariate meta analysis data.csv")

# meta sensitivity
fit_mv_meta <- rma.mv(yi     = yi, 
                      V      = V, 
                      random = ~ 1 | Experiment, 
                      data   = combined_results_trimmed,
